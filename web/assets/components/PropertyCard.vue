...
<template>
  <div class="w-full lg:w-4/5 mx-auto my-5">
    <div class="z-0 order-1 md:order-2 relative w-full md:w-2/5 lg:w-1/2 h-80 md:h-full overflow-hidden rounded-lg md:rounded-none md:rounded-r-lg cursor-pointer" @click="$router.push(`/property?id=${id}`)">
      <!-- Image -->
      ...
      <div class="md:hidden absolute inset-0 h-full p-6 pb-6 flex  justify-start items-start bg-gradient-to-b from-transparent via-transparent to-gray-900">
        <h3 class="w-full font-bold text-2xl text-white leading-tight mb-2">{{ location }}</h3>
    ...
    <div class="relative rounded-lg flex flex-col md:flex-row items-center md:shadow-xl drop-shadow-sm md:h-80 mx-2 border border-gray-200">
      <!-- Info -->
      <div class="w-full md:w-2/5 lg:w-1/2 h-full flex items-center px-4 cursor-pointer bg-white rounded-lg md:rounded-none md:rounded-l-lg shadow-xl md:shadow-none" @click="$router.push(`/property?id=${id}`)">
        <div class="flex flex-col space-y-2 " >
          <div class="hidden md:flex flex-col">
            <p class=" text-2xl font-semibold">
              {{ location }}
            </p>
            <p class="text-sm font-normal text-gray-300">Gauteng</p>  
          </div>
          <!-- Costs -->
          <div>
            <p class="hidden md:block text-sm uppercase font-semibold text-gray-400">Purchase Price</p>
            <p class="hidden md:block font-bold text-2xl text-gray-700">{{ helpers.getCurrencyString(purchasePrice) }}</p>
          </div>
          <div class="mb-1 overflow-hidden break-words whitespace-pre">
            <p class="text-sm uppercase font-semibold text-gray-400" v-if="helpers.showDescription(description)">
              Description
            </p>
            <p class="text-gray-600 text-justify h-24 break-words whitespace-pre-wrap">{{ description }}</p>
          </div>
          <!-- Icons -->
      ...
      <!-- Image -->
      <div class="w-full md:w-2/5 lg:w-1/2 h-full object-fill object-center bg-blue-400 bg-opacity-30 bg-cover bg-bottom" style="background-image: url( https://images.unsplash.com/photo-1583608205776-bfd35f0d9f83?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=MXwxfDB8MXxhbGx8fHx8fHx8fA&ixlib=rb-1.2.1&q=80&w=1080&utm_source=unsplash_source&utm_medium=referral&utm_campaign=api-credit ); background-blend-mode: multiply;">
        <div class="flex justify-end">
          <svg class="h-7 w-7 mt-4 mr-4 text-white fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
            <path d="M12.76 3.76a6 6 0 0 1 8.48 8.48l-8.53 8.54a1 1 0 0 1-1.42 0l-8.53-8.54a6 6 0 0 1 8.48-8.48l.76.75.76-.75zm7.07 7.07a4 4 0 1 0-5.66-5.66l-1.46 1.47a1 1 0 0 1-1.42 0L9.83 5.17a4 4 0 1 0-5.66 5.66L12 18.66l7.83-7.83z"></path>
          </svg>
